...
async function initAdminPage() {
  ...
  renderAdminCharts(); // Fallback to mock data for charts if no backend endpoints exist
  CartStore.updateBadge();
}
...
function renderUsersTable(users) {
  ...
  if (!tbody) return;
  if (!users || !users.length) { tbody.innerHTML = '<tr><td colspan="6">No users found.</td></tr>'; return; }
